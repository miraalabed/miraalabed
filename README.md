<!-- Profile README for github.com/miraalabed -->

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=230&color=0:6C63FF,50:8E7CFF,100:57C5B6&text=Hi%20there,%20I'm%20Mira%20Al-Abed%20%F0%9F%91%8B&fontSize=38&fontColor=ffffff&fontAlignY=38&desc=Computer%20Science%20Graduate%20%7C%20Software%20Developer&descAlignY=59&animation=fadeIn" alt="Mira Al-Abed banner" />

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&duration=3000&pause=900&color=6C63FF&center=true&vCenter=true&width=700&lines=Computer+Science+Graduate+from+Birzeit+University;Junior+Software+Developer;Python+%7C+Java+%7C+Flutter+%7C+Web+Development;Interested+in+AI+and+Computer+Vision;Always+learning%2C+building%2C+and+improving" alt="Typing introduction" />
</a>

<br>

<a href="https://github.com/miraalabed">
  <img src="https://komarev.com/ghpvc/?username=miraalabed&label=Profile%20Views&color=6C63FF&style=for-the-badge" alt="Profile views" />
</a>
<a href="https://github.com/miraalabed?tab=followers">
  <img src="https://img.shields.io/github/followers/miraalabed?label=Followers&style=for-the-badge&color=57C5B6" alt="GitHub followers" />
</a>
<a href="https://github.com/miraalabed?tab=repositories">
  <img src="https://img.shields.io/badge/Open%20to%20Work-Yes-success?style=for-the-badge" alt="Open to work" />
</a>

</div>

---

## 👩‍💻 About Me

```javascript
const mira = {
  education: "B.Sc. in Computer Science — Birzeit University",
  location: "Ramallah, Palestine",
  role: "Junior Software Developer",
  interests: [
    "Software Engineering",
    "Artificial Intelligence",
    "Computer Vision",
    "Web Development",
    "Mobile Development"
  ],
  strengths: [
    "Problem Solving",
    "Data Structures & Algorithms",
    "Object-Oriented Programming",
    "Teamwork",
    "Fast Learning"
  ],
  status: "Open to junior roles, internships, and freelance opportunities"
};
```

- 🎓 Computer Science graduate from **Birzeit University**
- 💻 Interested in building practical software and solving real-world problems
- 🧠 Strong foundation in **OOP, Data Structures, Algorithms, and Database Systems**
- 📱 Experience developing **mobile, web, desktop, and computer-vision applications**
- 🔐 Built privacy-focused software using face detection and recognition
- 🌱 Currently improving my skills in software engineering, AI, and clean architecture
- 🤝 Comfortable working independently and within collaborative teams
- 💼 Open to **Junior Software Developer, Python Developer, Java Developer, Web Developer, Mobile Developer, and AI-related roles**

---

## 🛠️ Languages & Tools

<div align="center">

### Programming Languages

<img src="https://skillicons.dev/icons?i=python,java,javascript,dart,php,c,html,css&perline=8" alt="Programming languages" />

<br><br>

### Frameworks, Platforms & Development Tools

<img src="https://skillicons.dev/icons?i=flutter,androidstudio,git,github,vscode,linux,windows,sqlite&perline=8" alt="Frameworks and tools" />

</div>

### Technical Knowledge

<table>
<tr>
<td width="50%" valign="top">

**Software Development**
- Object-Oriented Programming
- Clean and modular code
- Debugging and testing
- File handling
- Version control with Git
- Technical documentation

</td>
<td width="50%" valign="top">

**Computer Science**
- Data Structures
- Algorithms
- Dynamic Programming
- Graph Algorithms
- Database Systems
- Problem Solving

</td>
</tr>
<tr>
<td width="50%" valign="top">

**Development Areas**
- Python applications
- Java applications
- Android development
- Flutter development
- Front-end development
- PHP web development

</td>
<td width="50%" valign="top">

**AI & Computer Vision**
- Face detection
- Face recognition
- Camera integration
- Threat assessment logic
- Privacy-focused systems
- Model integration

</td>
</tr>
</table>

---

## 🚀 Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### 🔐 [Privacy Guard](https://github.com/miraalabed/Privacy-Guard)

A privacy-focused Python application that uses camera-based face detection and recognition for privacy monitoring and threat assessment.

**Key features**
- Modular Python architecture
- Face detection and recognition
- Camera integration
- Privacy response system
- Local database support
- Windows and Linux setup

**Tech:** `Python` `Computer Vision` `PyQt5` `SQLite`

</td>
<td width="50%" valign="top">

### 💼 [Job Search Website](https://github.com/miraalabed/-Job-Search-Website)

A web platform where users can search for jobs, apply, upload resumes, and manage applications.

**Key features**
- Job search workflow
- Resume uploading
- User functionality
- Admin job management
- Application management

**Tech:** `PHP` `HTML` `CSS` `Web Development`

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🗺️ [Graph Project](https://github.com/miraalabed/Graph_Project)

An application that finds the best route between capital cities based on time, cost, or distance.

**Key features**
- Dijkstra's algorithm
- Graph data structures
- Route optimization
- Multiple comparison criteria

**Tech:** `Java` `Graphs` `Algorithms`

</td>
<td width="50%" valign="top">

### 🗜️ [Huffman Coding Project](https://github.com/miraalabed/HaffmanCodeProject)

A file compression and decompression application based on Huffman Coding.

**Key features**
- Frequency table generation
- Huffman tree construction
- Binary code generation
- File compression and decompression

**Tech:** `Java` `Trees` `Priority Queues`

</td>
</tr>

<tr>
<td width="50%" valign="top">

### 🎮 [Dynamic Programming Game](https://github.com/miraalabed/Algo_proj1)

A turn-based coin collection game that calculates optimal moves using dynamic programming.

**Key features**
- Player vs. player
- Player vs. computer
- Optimal move calculation
- Algorithmic decision-making

**Tech:** `Java` `Dynamic Programming`

</td>
<td width="50%" valign="top">

### 📱 [Perfume Shopping App](https://github.com/miraalabed/myApp)

An Android shopping application for browsing perfume products with images, descriptions, and prices.

**Key features**
- Android user interface
- Product browsing
- Product details
- Mobile shopping experience

**Tech:** `Java` `Android`

</td>
</tr>
</table>

<div align="center">

<a href="https://github.com/miraalabed?tab=repositories">
  <img src="https://img.shields.io/badge/View%20All%20Projects-181717?style=for-the-badge&logo=github&logoColor=white" alt="View all repositories" />
</a>

</div>

---

## 📊 GitHub Activity

<div align="center">

<img width="49%" src="https://github-readme-stats.vercel.app/api?username=miraalabed&show_icons=true&hide_border=true&title_color=6C63FF&icon_color=57C5B6&text_color=555555&bg_color=00000000&include_all_commits=true&count_private=true" alt="Mira's GitHub statistics" />

<img width="45%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=miraalabed&layout=compact&hide_border=true&title_color=6C63FF&text_color=555555&bg_color=00000000&langs_count=8" alt="Mira's most used languages" />

<br>

<img width="70%" src="https://streak-stats.demolab.com?user=miraalabed&hide_border=true&ring=6C63FF&fire=57C5B6&currStreakLabel=6C63FF&sideLabels=555555&dates=777777&background=00000000" alt="Mira's GitHub streak" />

<br>

<img width="95%" src="https://github-readme-activity-graph.vercel.app/graph?username=miraalabed&bg_color=ffffff00&color=6C63FF&line=57C5B6&point=6C63FF&area=true&hide_border=true" alt="Mira's contribution graph" />

</div>

---

## 🎓 Education

### Birzeit University

**Bachelor of Science in Computer Science**

Relevant coursework and knowledge:

`Programming` • `Object-Oriented Programming` • `Data Structures` • `Algorithms` • `Database Systems` • `Operating Systems` • `Artificial Intelligence` • `Software Engineering` • `Computer Networks` • `Web Development`

---

## 🌟 Professional Strengths

- Analytical thinking and structured problem solving
- Ability to learn new technologies quickly
- Writing organized and maintainable code
- Strong attention to detail
- Team collaboration and clear communication
- Responsibility and commitment to deadlines
- Technical research and documentation
- Presenting and explaining software projects

---

## 📫 Connect With Me

<div align="center">

<a href="https://www.linkedin.com/in/mira-alabed-338553313?utm_source=share_via&utm_content=profile&utm_medium=member_ios/">
  <img src="https://img.shields.io/badge/LinkedIn-Mira%20Al--Abed-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>
<a href="mailto:miraalabed21@gmail.com">
  <img src="https://img.shields.io/badge/Email-Contact%20Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
</a>
<a href="https://github.com/miraalabed">
  <img src="https://img.shields.io/badge/GitHub-miraalabed-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
</a>

<br><br>

**Open to junior roles, internships, freelance work, and collaborative projects.**

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0:57C5B6,50:8E7CFF,100:6C63FF" width="100%" alt="Footer" />

### Thanks for visiting my profile! 💜

</div>
